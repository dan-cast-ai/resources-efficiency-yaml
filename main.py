from constants import CLUSTERS_URL, CLUSTER_NAME, WORKLOADS_URL_POST, EXTERNAL_CLUSTERS_URL, \
    ...
    EFFICIENCY_URL_START_POST, GB_BYTES
...
def get_list(url):
    headers = {
        ...
        "X-API-Key": f"{os.getenv('CAST_AI_API_KEY')}"
    }
...
def create_yaml(workload_efficiency_list, start, end):
    resources = []
    with open(f'resources_efficiency_{datetime.now().strftime("%Y%m%d%H%M%S")}.yaml', 'w') as file:
        for item in workload_efficiency_list:
            ...
            file.write(f"--- {item['workloadName']} --- {item['workloadType']} --- {item['namespace']} ---\n")
            for container_raw_data in resources_raw_data['containers']:
                ...
                if len(container_raw_data['items']):
                    request = {
                        "memory": convert_gb_to_bytes(container_raw_data['items'][0]['info']['recommendation']['memoryGib']),
...
if __name__ == '__main__':
    ...
    yaml = create_yaml(workload_efficiency_list, start_time, end_time)
    print("")
    with open(f'resources_efficiency_{datetime.now().strftime("%Y%m%d%H%M%S")}.yaml', 'w') as file:
        file.write(yaml)
